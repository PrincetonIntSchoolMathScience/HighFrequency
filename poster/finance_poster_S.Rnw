...
\subsection*{Others did}

In \cite{bollen2011twitter} researchers did:
\begin{itemize}
...
\item Prediction interval: 1 day
\item Results: 87.6 percentin predicting the daily up and own changes in the closing values of the DJIA and a reduction of the Mean Average Percentage Error by more than 6 percent.
\end{itemize}
...
\begin{enumerate}
\item Caputure and Clean Tweets
...
I choose Twitter to be the main social media platform to collect twitter feeds. The Twitter API allows user to collect tweets based on different types of filters. I choose to search for tweets that contain certain key words. The key words are the independent variables in the research. And I have chosen stock market to be the keyword in my research. After collecting data, I proceed data cleaning by converting all files into txt form in order to be scored according to the sentiment analysis wordlist.Then, the google data is collected for stock price. The last section is the granger-causality test and running cross validation to verify the relationship between emotions and stock price.

%------------------------------------------------

\subsection*{Coding Section}
...
\item In order to process the tweets we collect, mainly scoring the emotion of each tweet. In the scoreing function, we get rid of all special symbols, starting users name,  punctuation, digit, link, capital letter. It will be easier for judging whether one string contains on string. 
...
\item In the scoring function, we use a for loop to search for the positive or negtive word in one specific string. It is fairly easy to achieve by lookup each word in the word list and record number of positive words and negtive words and calculate the difference bweteen them. However, we also have to get rid of the repetitive count when two positive words or two negative words contain one of the other. The for loop firstly run through negative words and delete repetitive terms and executes in the same way for positive words. 

\item Google provides last 15 days of data with one minutes interval. We only have to contruct the url string to access the txt format of table and then format them together. We first go to the url, get organize the data into columns. We then search for the time stamp, which is in POSCIX format. Then we convert it to EST and get two columns from this data frame: price closed and time for the future analysis 
\\
...
\begin{itemize}
\item After runing the 10 days interval data, the p-value is $9.03*10^{-11}$ and the $R^2$ is around 0.27. It indicates that there is an moderate correlation between emotional factors and stock market price from the perspective of this research. However, it may just be the coincidence of the data set. There are still a few improvements of the research. 
\item How we actually calculate the emotion is flawed. Think more about the content of the scoring mechanism, we will naturally realize that our emotions do not purely depend on the word we choose, but also include the content of the sentences. 
\item The keywords search should be more generalized
\item Tweets capturing accuracy
\end{itemize}
...
The next stage of the research will be improving dictions by adapting a wordlist that differentiates the emotional level of the words. Or using maching learning to analyze the emotion by Google Opinion Finder. The keywords search needs improvement as "stock market" is not the only thing that is related to the stock market. A specific company analysis may also be run considering the public exposure of each company is different. 
...
I am thankfull to PRISMS for providing time and support for my endevours.
%----------------------------------------------------------------------------------------
